...
export default function DataPage() {
  ...
  <div className="actions" style={{ display: 'flex', gap: 8 }}>
          ...
          }}><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M12 3v10" stroke="#0f172a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M8 7l4-4 4 4" stroke="#0f172a" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> Export CSV</button>
          <button className="btn primary icon" onClick={() => navigate('/chart')}><svg width="16" height="16" viewBox="0 0 24 24" fill="none"><path d="M3 3v18h18" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/><path d="M18 9l-6 6-3-3" stroke="white" strokeWidth="1.5" strokeLinecap="round" strokeLinejoin="round"/></svg> Lihat Chart</button>
        </div>
      </div>
      ...
      {total === 0 ? (
        ...
        <>
          <DataTable columns={columns} data={pageSlice} height={520} rowHeight={48} onRowClick={(r) => alert(`Klik ${r.kode} - ${r.nama}`)} sortable sortState={sortState} onSortChange={(s) => { setSortState(s); setPage(1) }} />
          <div style={{ display: 'flex', justifyContent: 'space-between', marginTop: 12, alignItems: 'center' }}>
            <Pagination page={page} perPage={perPage} total={total} onChange={(p, newPerPage) => { if (newPerPage) { setPerPage(newPerPage); setPage(1); } else setPage(p) }} />
            <div style={{ color: 'var(--muted)' }}>Menampilkan {pageStart + 1} - {Math.min(pageStart + perPage, total)} dari {total} baris</div>
          </div>
        </>
      )}
        </div>
